...
export default function App() {
  ...
  const data = useMemo(() => effectiveCollection(baseData, plotAttrs, plotGeom, merges), [baseData, plotAttrs, plotGeom, merges]);
  ...
  return (
    <>
      <MapView data={data} projects={projects} landUses={landUses} />
      <TopBar onOpenAdmin={() => openAdmin()} onOpenDevPlan={() => setDevOpen(true)} />
      ...
      <AdminConsole
        open={adminOpen} onClose={() => { setAdminOpen(false); setEditCode(null); }}
        editCode={editCode} data={data} projects={projects} landUses={landUses}
      />
      <DevPlanView
        open={devOpen} onClose={() => setDevOpen(false)}
