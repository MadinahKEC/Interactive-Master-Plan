...
### 7) فعّل الاستعادة من داخل الموقع (تطبيق ويب)
لتظهر النسخ داخل التطبيق (لوحة التحكم ▸ النسخ الاحتياطية) وتستعيد بضغطة:
1. في **Script properties** أضِف `WEB_SECRET` بأي قيمة سرّية قوية (مثلاً `kec-bk-9f3Kd82m`).
2. **Deploy ▸ New deployment** → اختر النوع **Web app**:
   - **Execute as:** Me
   - **Who has access:** Anyone
   - اضغط **Deploy** وامنح الأذونات، ثم انسخ **Web app URL** (ينتهي بـ `/exec`).
3. في الموقع: سجّل الدخول كمدير → **لوحة التحكم ▸ النسخ الاحتياطية** → الصق **الرابط** و**الرمز السري** (نفس `WEB_SECRET`) → **اتصال وتحديث**.
4. ستظهر قائمة النسخ اليومية. اضغط **استعادة** بجانب أي يوم لاستبداله بالبيانات الحالية (مع رسالة تأكيد). تُخزَّن هذه الإعدادات في هذا المتصفّح فقط.

---

## الاستعادة عند الحاجة (نادراً)
**الطريقة الأسهل:** من داخل الموقع كما في الخطوة 7 (استعراض واختيار واستعادة بضغطة).

**بديل يدوي عبر Apps Script:**
